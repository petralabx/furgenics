...
2b. **Done 2026-08-14.** Pushed `snippets/buy-buttons.liquid` (alignment fix — the step-2 restyle had been defeated by `assets/mx-style.css` global overrides: `.button` forced solid teal + arrow padding, `.details .product-form__submit.button { width: auto }`, Dawn's `:after` border mechanism disabled) to `#152547065995` only (`shopify theme push --nodelete --only`). Live `#150922428555` unchanged, nothing published. Rendered preview confirmed at 1440px and 390px: Add to cart full-width solid teal (`rgb(1, 69, 102)`) with arrow at `calc(100% - 20px)`; Apply for salon pricing full-width outlined secondary stacked below; left/right edges identical. Consider pulling + versioning `assets/mx-style.css` (P2) — it is the source of repeated styling surprises.
2c. **PENDING PUSH — Stephen's visual feedback round (2026-08-14 later).** (i) FAQ section ran edge-to-edge (the theme's custom-liquid section has no width container, and this theme's `.page-width` is full-width + 50px side gutter, which our inline `padding: 32px 0 0` was zeroing) — `furgenics-product-faqs.liquid` now uses `page-width` + `padding-top` only; verified via DOM edit on the rendered preview. (ii) The compact testimonial card was reverted — Stephen prefers the original full "Loved by Pets, Endorsed by the Best" layout; restored in `main-product.liquid` with alt text + lazy loading (portrait still hardcoded from a foreign store CDN — swap when photography lands). Push list: `snippets/furgenics-product-faqs.liquid`, `sections/main-product.liquid`.
